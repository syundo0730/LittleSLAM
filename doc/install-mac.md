## LittleSLAMの使い方（Macの場合）

### (1) 関連ソフトウェアのインストール

- C++コンパイラ(gcc)、Boost、Eigen、CMake   
以下のコマンドで、まとめてインストールします。

</code></pre>
<pre><code> $ brew install cmake boost eigen 
</code></pre>
...
- AquaTerm
macでは、gnuplotの出力先にaquatermを使うと綺麗に表示されるため、AquaTermをインストールします。

[AquaTerm](https://sourceforge.net/projects/aquaterm/)より.dmgファイルをダウンロードしてインストールします。

以下のコマンドで、出力先をAquaTermに指定してgnuplotをインストールします。
</code></pre>
<pre><code> $ brew install gnuplot --with-aquaterm
</code></pre>
### (2) LittleSLAMのインストール
